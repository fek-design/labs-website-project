...
export function InventoryManager() {
  ...
  // Filters (Makerspace is global default)
  ...
  // New Item Form Data (Auto-generated deterministic tag)
  ...
  const [selectedPrimaryTag, setSelectedPrimaryTag] = useState<string>("general-tools");
  ...
  }, [selectedLab, selectedType, selectedStatus, selectedTag, searchQuery]);

  useEffect(() => {
    fetchInventory();
  }, [fetchInventory]);

  // Update preview tag when lab or category changes
  useEffect(() => {
    if (showAddDrawer) {
      generateAssetTag({
        labSlug: newLabSlug,
        tagSlug: selectedPrimaryTag,
      }).then((tag) => setPreviewTag(tag));
    }
  }, [showAddDrawer, newLabSlug, selectedPrimaryTag]);
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createInventoryItem({
        ...
        tagSlugs: selectedPrimaryTag ? [selectedPrimaryTag] : [],
      });
  ...
  return (
    <div className="bg-[#141414] border border-[#262626] rounded-3xl p-6 shadow-2xl font-mono text-white">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-bold tracking-tight text-white">
              Inventory & Macro-Facility Catalog
            </h2>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Macro-Lab facility tracking, automated deterministic tagging, and rapid item management
          </p>
        </div>
      ...
      {/* Filter Bar with Macro-Labs */}
      <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 pb-6 border-b border-[#262626]">
        {/* Macro Facility Lab Filter */}
        <div>
          <label className="text-[10px] text-zinc-500 uppercase block mb-1">Macro Facility Lab</label>
          <select
            value={selectedLab}
            onChange={(e) => setSelectedLab(e.target.value)}
            className="w-full bg-[#0D0D0D] border border-[#262626] text-[#009FE3] text-xs rounded-xl p-2 outline-none font-bold"
          >
        ...
        <div>
          <label className="text-[10px] text-zinc-500 uppercase block mb-1">Hardware Type</label>
          <select
            value={selectedType}
            onChange={(e) => setSelectedType(e.target.value)}
            className="w-full bg-[#0D0D0D] border border-[#262626] text-white text-xs rounded-xl p-2 outline-none"
          >
            <option value="ALL">All Types</option>
            <option value="BORROWABLE_GEAR">Borrowable Gear</option>
            <option value="STATIC_MACHINE">Static Machine</option>
          </select>
        </div>

        {/* Operational Status Filter */}
        <div>
          <label className="text-[10px] text-zinc-500 uppercase block mb-1">Status</label>
          <select
        ...
        {/* Category Tag Filter */}
        <div>
          <label className="text-[10px] text-zinc-500 uppercase block mb-1">Category Tag</label>
          <select
            value={selectedTag}
        ...
        {/* Quick Search */}
        <div>
          <label className="text-[10px] text-zinc-500 uppercase block mb-1">Search</label>
          <input
            type="text"
      ...
      <div className="mt-4 overflow-x-auto">
        {items.length === 0 ? (
          <div className="text-center py-16 text-zinc-500 text-xs">
            No inventory assets match the current filter selection.
          </div>
        ) : (
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="text-zinc-500 border-b border-[#262626]">
                ...
                <th className="py-3 px-3 font-semibold uppercase tracking-wider">Circulation / State</th>
                  ...
                  <tr key={item.id} className="hover:bg-[#1a1a1a] transition-colors">
                    ...
                    <td className="py-3 px-3">
                      {activeLoan ? (
                        ...
                        <span className="text-emerald-400 text-[11px] font-bold">✓ Ready for loan</span>
                      ) : (
      ...
      {/* Add New Item Modal / Drawer with Auto Tagging */}
      <AnimatePresence>
        {showAddDrawer && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div
              ...
              className="bg-[#141414] border border-[#FFED00] rounded-3xl p-6 max-w-lg w-full shadow-2xl font-mono text-xs max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between pb-3 border-b border-[#262626]">
                <div>
                  <h3 className="text-sm font-bold text-[#FFED00]">Register New Inventory Asset</h3>
                  <p className="text-[11px] text-zinc-400">Deterministic auto-tagging enabled</p>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleCreateSubmit} className="mt-4 space-y-3">
                {/* Auto-Generated Asset Tag Display (Manual Input Disabled) */}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-zinc-400 block mb-1 font-bold">Category Tag</label>
                    <select
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Sony Alpha FX3 Cinema Camera"
                    value={newName}
                ...
                <div>
                  ...
                  >
                    <option value="BORROWABLE_GEAR">Borrowable Gear (Rentals / Medialab)</option>
                    <option value="STATIC_MACHINE">Static Machine (In-House / Makerspace)</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    rows={2}
                    placeholder="Kit accessories, operating constraints, or serial numbers..."
                    value={newNotes}
